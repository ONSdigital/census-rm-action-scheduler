...
public class ActionRuleProcessorTest {
  ...
  public void testExecuteAllCases() {
    ...
    when(caseRepository.findByActionPlanId(actionPlan.getId().toString()))
        .thenReturn(Arrays.stream(cases));
    ...
    verify(rabbitTemplate, times(cases.length))
        .convertAndSend(
  ...
  private List<ActionInstruction> getExpectedActionInstructions(
      Case[] cases, ActionRule actionRule, List<ActionInstruction> actionInstructions) {
    ...
    for (int i = 0; i < cases.length; i++) {
      expectedActionInstructions.add(
          getExpectedActionInstruction(
              cases[i],
              cases[i].getCaseId().toString() + "uac",
              actionRule,
